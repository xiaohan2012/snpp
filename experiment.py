...
from snpp.cores.lowrank import partition_graph, alq_spark
...
if __name__ == "__main__":
    ...
    if True:
        ...
        part, iter_preds, status = iterative_approach(
            ...
            budget_allocation_kwargs=dict(const=1500),
    ...
    X, Y = alq_spark(A, k=k, sc=spark_context,
                     lambda_=lambda_, iterations=max_iter,
                     seed=random_seed)

    Xb, Yb = spark_context.broadcast(X), spark_context.broadcast(np.transpose(Y))
    lr_preds = spark_context.parallelize(remaining_targets).map(
    ...
    truth = set((i, j, test_m[i, j]) for i, j in test_idx_sorted)
    preds = lr_preds | iter_preds | ud_preds
    assert len(preds) == len(truth)
    len(truth.intersection(preds)) / len(truth)
